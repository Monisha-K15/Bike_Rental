...
# ── Verified column list from actual model (best_model_pipeline.pkl) ─────────
# The model was trained with string categories, not integers, and includes temp_bin
EXPECTED_COLS = [
    ...
    # yr as string category '2011'/'2012' → drop_first removes '2011' → yr_2012
    'yr_2012',
    # holiday as string category 'no'/'yes' → drop_first removes 'no' → holiday_yes
    'holiday_yes',
    # weekday category (int) → drop_first removes 0 → weekday_1…weekday_6
    'weekday_1', 'weekday_2', 'weekday_3', 'weekday_4', 'weekday_5', 'weekday_6',
    # workingday as string 'no'/'working day' → drop_first removes 'no'
    ...
    # temp_bin categories (NOT dropped in training - included in model)
    'temp_bin_Cold', 'temp_bin_Mild', 'temp_bin_Warm', 'temp_bin_Hot',
    # yr_num as string '2011'/'2012' → drop_first removes '2011' → yr_num_2012
    'yr_num_2012',
    ...
    # weather_group → same string values as weathersit
    #   drop_first removes 'clear' → keeps heavy rain/light snow/mist
    'weather_group_heavy rain', 'weather_group_light snow', 'weather_group_mist',
]
...
def build_input_df(season, yr, mnth, hr, holiday, weekday, workingday,
    ...
    # weather_group: same as weathersit (replicates notebook behavior)
    weather_group_val = weathersit_str

    # temp_bin calculation (5 bins, drop_first removes first bin)
    # bins defined to match notebook: Very Cold, Cold, Mild, Warm, Hot
    if temp < 5:
        temp_bin = 'Very Cold'
    elif temp < 15:
        temp_bin = 'Cold'
    ...
    # Convert integer inputs to string categories as expected by model
    yr_str = '2012' if yr == 1 else '2011'
    holiday_str = 'yes' if holiday == 1 else 'no'
    workingday_str = 'working day' if workingday == 1 else 'no'
    ...
    row = {
        ...
        # String categories (as used during model training)
        'yr'           : pd.Categorical([yr_str], categories=['2011', '2012']),
        'holiday'      : pd.Categorical([holiday_str], categories=['no', 'yes']),
        'weekday'      : pd.Categorical([weekday], categories=list(range(7))),
        'workingday'   : pd.Categorical([workingday_str], categories=['no', 'working day']),
        'season'       : pd.Categorical([SEASON_MAP[season]],
                                        categories=['fall', 'spring', 'summer', 'winter']),
        'weathersit'   : pd.Categorical([weathersit_str],
                                        categories=['clear', 'heavy rain', 'light snow', 'mist']),
        'temp_bin'     : pd.Categorical([temp_bin],
                                        categories=['Very Cold', 'Cold', 'Mild', 'Warm', 'Hot']),
        ...
        # weather_group same values as weathersit
    ...
    cat_cols = ['yr', 'holiday', 'weekday', 'workingday',
                'season', 'weathersit', 'temp_bin', 'time_of_day', 'weather_group', 'yr_num']
