...
WEATHER_GROUP_MAP = {1: 'Clear', 2: 'Mist/Cloudy', 3: 'Light Rain/Snow', 4: 'Heavy Rain/Snow'}

# All columns X had at fit time (after leakage cols dropped, get_dummies applied)
EXPECTED_COLS = [
    # Numeric pass-through
    'temp', 'atemp', 'hum', 'windspeed', 'hr', 'is_weekend', 'temp_diff',
    # yr integer category → yr_1
    'yr_1',
    # mnth integer category (drop first = 1) → mnth_2 … mnth_12
    'mnth_2', 'mnth_3', 'mnth_4', 'mnth_5', 'mnth_6',
    'mnth_7', 'mnth_8', 'mnth_9', 'mnth_10', 'mnth_11', 'mnth_12',
    # holiday integer category → holiday_1
    'holiday_1',
    # weekday integer category (drop first = 0) → weekday_1 … weekday_6
    'weekday_1', 'weekday_2', 'weekday_3', 'weekday_4', 'weekday_5', 'weekday_6',
    # workingday integer category → workingday_1
    'workingday_1',
    # season string category (drop first = 'fall') → season_spring, season_summer, season_winter
    'season_spring', 'season_summer', 'season_winter',
    # weathersit string category (drop first = 'clear') → weathersit_heavy rain, _light rain, _mist
    'weathersit_heavy rain', 'weathersit_light rain', 'weathersit_mist',
    # time_of_day category (drop first = 'Afternoon') → Evening, Morning, Night
    'time_of_day_Evening', 'time_of_day_Morning', 'time_of_day_Night',
    # weather_group string (drop first = 'Clear') → Heavy Rain/Snow, Light Rain/Snow, Mist/Cloudy
    'weather_group_Heavy Rain/Snow', 'weather_group_Light Rain/Snow', 'weather_group_Mist/Cloudy',
    # yr_num string (drop first = '0') → yr_num_1
    'yr_num_1',
]
...
def build_input_df(season, yr, mnth, hr, holiday, weekday, workingday,
    ...
    row = {
        ...
        # Integer categories (will produce _1 dummy)
        'yr'           : pd.Categorical([yr],          categories=[0, 1]),
        'mnth'         : pd.Categorical([mnth],        categories=list(range(1, 13))),
        ...
        # String categories (will produce named dummies)
        'season'       : pd.Categorical([SEASON_MAP[season]],
                                        categories=['fall', 'spring', 'summer', 'winter']),
        'weathersit'   : pd.Categorical([WEATHERSIT_MAP[weathersit]],
                                        categories=['clear', 'heavy rain', 'light rain', 'mist']),
        # Engineered string features
        'time_of_day'  : pd.Categorical([tod],
                                        categories=['Afternoon', 'Evening', 'Morning', 'Night']),
        'weather_group': pd.Categorical([WEATHER_GROUP_MAP[weathersit]],
                                        categories=['Clear', 'Heavy Rain/Snow',
                                                    'Light Rain/Snow', 'Mist/Cloudy']),
        'yr_num'       : pd.Categorical([str(yr)], categories=['0', '1']),
    }

    df_row = pd.DataFrame({k: (v if isinstance(v, pd.Categorical) else [v])
                           for k, v in row.items()})

    # Apply get_dummies exactly as the notebook did
    cat_cols = ['yr', 'mnth', 'holiday', 'weekday', 'workingday',
