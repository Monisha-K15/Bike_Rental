...
# Notebook pipeline:
#   1. season stored as STRING category: 'fall','spring','summer','winter'
#      → get_dummies(drop_first=True) drops 'fall' → produces season_spring, season_summer, season_winter
#   2. weathersit stored as STRING category: 'clear','heavy rain','light rain','mist'
#      → produces weathersit_heavy rain, weathersit_light rain, weathersit_mist
#   3. holiday/workingday stored as INTEGER category: 0/1
#      → produces holiday_1, workingday_1
#   4. yr stored as INTEGER category: 0/1 → yr_1
#   5. weekday stored as INTEGER category: 0-6 → weekday_1 … weekday_6
#   6. mnth stored as INTEGER category: 1-12 → mnth_2 … mnth_12
#   7. Engineered: time_of_day (Afternoon/Evening/Morning/Night) → drops Afternoon
#   8. Engineered: weather_group (Clear/Heavy Rain-Snow/Light Rain-Snow/Mist-Cloudy) → drops Clear
#   9. Engineered: yr_num (string '0'/'1') → yr_num_1
#   10. temp_bin DROPPED (leakage col); is_weekend & temp_diff kept as numeric

SEASON_MAP = {1: 'spring', 2: 'summer', 3: 'fall', 4: 'winter'}
WEATHERSIT_MAP = {1: 'clear', 2: 'mist', 3: 'light rain', 4: 'heavy rain'}

# ── Verified column list (simulated from notebook execution) ──────────────────
#
# Notebook state at pd.get_dummies(df, drop_first=True) [Section 9]:
#
#  mnth      → int64   (Section 5.4 does pd.to_numeric, stripping category dtype)
#  weekday   → category(int) (.isin() does NOT strip category dtype)
#  yr        → category(int 0/1)
#  holiday   → category(int 0/1)
#  workingday→ category(int 0/1)
#  season    → category(str: fall/spring/summer/winter)
#  weathersit→ category(str: clear/heavy rain/light rain/mist)
#  time_of_day → category(labels=['Night','Morning','Afternoon','Evening'])
#               drop_first removes FIRST label = 'Night' → keeps Morning/Afternoon/Evening
#  weather_group → notebook maps weathersit with INTEGER keys {1:...,2:...} but
#               weathersit values are STRINGS → replace has no effect → weather_group
#               retains original weathersit category values (clear/mist/light rain/heavy rain)
#               drop_first removes 'clear' → keeps heavy rain/light rain/mist
#  yr_num    → plain str ('0'/'1'); drop_first removes '0' → keeps yr_num_1
#
EXPECTED_COLS = [
    # Numeric (not dummy-encoded)
    'temp', 'atemp', 'hum', 'windspeed', 'hr', 'mnth',
    'is_weekend', 'temp_diff',
    # yr category (int) → drop_first removes 0 → yr_1
    'yr_1',
    # holiday category (int) → holiday_1
    'holiday_1',
    # weekday category (int) → drop_first removes 0 → weekday_1…weekday_6
    'weekday_1', 'weekday_2', 'weekday_3', 'weekday_4', 'weekday_5', 'weekday_6',
    # workingday category (int) → workingday_1
    'workingday_1',
    # season string category → drop_first removes 'fall' → spring/summer/winter
    'season_spring', 'season_summer', 'season_winter',
    # weathersit string category → drop_first removes 'clear'
    'weathersit_heavy rain', 'weathersit_light rain', 'weathersit_mist',
    # time_of_day → categories=['Night','Morning','Afternoon','Evening']
    #   drop_first removes first = 'Night' → keeps Morning/Afternoon/Evening
    'time_of_day_Morning', 'time_of_day_Afternoon', 'time_of_day_Evening',
    # weather_group → same string values as weathersit (notebook bug: int keys don't map)
    #   drop_first removes 'clear' → keeps heavy rain/light rain/mist
    'weather_group_heavy rain', 'weather_group_light rain', 'weather_group_mist',
    # yr_num plain str → drop_first removes '0' → yr_num_1
    'yr_num_1',
]
...
def build_input_df(season, yr, mnth, hr, holiday, weekday, workingday,
    ...
    # Time-of-day bucket — matches notebook: bins=[-1,6,12,18,24]
    # labels=['Night','Morning','Afternoon','Evening']
    # (-1,6]=Night  (6,12]=Morning  (12,18]=Afternoon  (18,24]=Evening
    if hr <= 6:
        tod = 'Night'
    elif hr <= 12:
        tod = 'Morning'
    ...
    # weathersit string value
    weathersit_str = WEATHERSIT_MAP[weathersit]

    # weather_group: notebook bug — replace({1:...,2:...}) on a string-category column
    # has no effect, so weather_group ends up with the same string as weathersit.
    # We replicate this exactly so column names match the trained model.
    weather_group_val = weathersit_str  # matches notebook's actual behaviour

    row = {
        # Plain numeric (mnth is int64 in notebook, NOT category, at encoding time)
        'temp'         : temp,
        'atemp'        : atemp,
        'hum'          : hum,
        'windspeed'    : windspeed,
        ...
        # Integer categories → get dummy-encoded
        'yr'           : pd.Categorical([yr],         categories=[0, 1]),
        'holiday'      : pd.Categorical([holiday],    categories=[0, 1]),
        'weekday'      : pd.Categorical([weekday],    categories=list(range(7))),
        'workingday'   : pd.Categorical([workingday], categories=[0, 1]),
        # String categories
        'season'       : pd.Categorical([SEASON_MAP[season]],
                                        categories=['fall', 'spring', 'summer', 'winter']),
        'weathersit'   : pd.Categorical([weathersit_str],
                                        categories=['clear', 'heavy rain', 'light rain', 'mist']),
        # time_of_day: explicit category order matters for drop_first
        'time_of_day'  : pd.Categorical([tod],
                                        categories=['Night', 'Morning', 'Afternoon', 'Evening']),
        # weather_group replicates notebook bug: same string values as weathersit
        'weather_group': pd.Categorical([weather_group_val],
                                        categories=['clear', 'heavy rain', 'light rain', 'mist']),
        # yr_num: plain string (not category)
        'yr_num'       : str(yr),
    }

    df_row = pd.DataFrame({k: ([v] if not isinstance(v, pd.Categorical) else v)
                           for k, v in row.items()})

    # Dummy-encode categorical columns (mnth stays as plain int, not encoded)
    cat_cols = ['yr', 'holiday', 'weekday', 'workingday',
                'season', 'weathersit', 'time_of_day', 'weather_group', 'yr_num']
